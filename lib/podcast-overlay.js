...
export async function renderSliceOverlays({
    ...
    // Caption chunks reveal sequentially — never overlap. The previous
    // implementation extended each chunk's enable window by +0.05s which
    // caused two chunks to be visible at the same y-position for 50ms,
    // mangling the text on screen. Now each chunk's window ends just
    // BEFORE the next one begins.
    const chunks = chunkText(sceneText, emphasis);
    const beat = sliceDuration / chunks.length;
    const GAP = 0.02; // safe margin so float arithmetic never overlaps
    for (let i = 0; i < chunks.length; i++) {
        const chunk = chunks[i];
        const t0 = beat * i;
        const t1 = i === chunks.length - 1
            ? sliceDuration
            : Math.max(t0 + 0.1, beat * (i + 1) - GAP);
