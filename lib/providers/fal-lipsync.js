...
// Default sync_mode: 'loop' so the video loops subtly to match the FULL
// audio length. 'cut_off' (the API default) truncates to the shorter of
// the two, which silently dropped scenes off the end of every podcast.
export async function lipsync({ videoUrl, audioUrl, model = 'lipsync-2-pro', syncMode = 'loop' }) {
    ensureConfigured();
