...
const SYSTEM = [
    'You script 9:16 podcasts where TWO HOSTS sit across from each other in ONE room and the edit cuts between their PROFILE / 3-quarter close-ups every 2-4s.',
    ...
    'speakers MUST include gender ("male"|"female") and look like REAL people — asymmetric features, real skin, lived-in clothing, NO celebrity likenesses, NOT CGI.',
    'speakers MUST include cameraSide: "left" means the speaker SITS on the LEFT half of the frame and their gaze points RIGHT off-frame at the other host. "right" means SITS on the RIGHT half and gaze points LEFT off-frame. The two speakers ALWAYS have OPPOSITE cameraSide values.',
    'IMPORTANT: hosts NEVER look at the camera — they look at each other across the table. The camera is a fly-on-the-wall observer.',
].join(' ');
...
export async function planPodcastEpisode({
    ...
    const schema = `{"showName":"...","brandCopy":"<show name CAPS>","productCopy":"<product name>","setting":"shared room: armchairs, lamp, microphones, bookshelf, walls, lighting","speakers":[{"id":"host","gender":"male|female","cameraSide":"left|right","persona":"...","appearance":"distinct face/hair/age, real-looking, asymmetric features, lived-in","wardrobe":"clothes worn in every shot"},{"id":"guest","gender":"male|female","cameraSide":"opposite of host","persona":"...","appearance":"different from host","wardrobe":"..."}],"hook":"cold open","scenes":[{"id":1,"speaker":"host","text":"≤10 words","imagePrompt":"only what changes (expression/gesture/hand)","emphasis":"word|null","productPop":false}]}`;
    ...
    // Normalise speakers — fill in gender / cameraSide if the model omitted them.
    const normalisedSpeakers = (parsed.speakers || []).map((s, idx) => ({
        ...s,
        gender: ['male', 'female'].includes(String(s.gender || '').toLowerCase())
            ? String(s.gender).toLowerCase()
            : (idx === 0 ? 'male' : 'female'),
        cameraSide: ['left', 'right'].includes(String(s.cameraSide || '').toLowerCase())
            ? String(s.cameraSide).toLowerCase()
            : (idx === 0 ? 'left' : 'right'),
    }));
    // Force cameraSide to be opposite for the two speakers.
    if (normalisedSpeakers.length >= 2 && normalisedSpeakers[0].cameraSide === normalisedSpeakers[1].cameraSide) {
        normalisedSpeakers[1].cameraSide = normalisedSpeakers[0].cameraSide === 'left' ? 'right' : 'left';
    }
    const finalSpeakers = normalisedSpeakers;
