...
const SYSTEM = [
    'You are a senior short-form podcast producer scripting a 9:16 vertical podcast where TWO HOSTS sit at TWO DIFFERENT podcast set-ups and the edit cuts back and forth between them every 2-4 seconds.',
    ...
    'Each scene is ONE beat: one short spoken line, max 10 words, that the speaker actually says into the microphone. Lines feel natural — interjections, reactions, "no way", "wait what", "[laughs]" allowed.',
    'The dialogue tells a story: cold-open hook → host introduces product → guest reacts in disbelief → they riff a feature or two → close on a CTA. Treat it as banter, not narration.',
    'imagePrompt locks visual identity for THAT speaker (face/hair/wardrobe/set/microphone/lighting). Repeat the locks every scene so the same speaker is recognizable. videoPrompt = subtle natural motion (talking, head tilt, hand gesture, leaning back, laughing).',
    'On scenes where the product NAME or a key feature is introduced, set "productPop":true so the UI shows a colorful brand sticker on top of the frame; otherwise omit it.',
    'Avoid celebrity likenesses; characters should read as anonymized stylized podcast hosts.',
].join(' ');
...
export async function planPodcastEpisode({
    ...
    const schema = `{"showName":"<repeat the input show name>","brandSticker":{"position":"top-left","shape":"speech bubble","copy":"<show name in caps>","palette":"#0E0E0E+#FFFFFF"},"productSticker":{"shape":"capsule|squiggle","copy":"<the actual product name from the brief>","palette":"#FF2945+#FFFFFF"},"speakers":[{"id":"host","persona":"...","appearance":"distinct face/hair/age/skin","wardrobe":"specific clothes","setting":"set A — specific props, lamp, bookshelf, lighting","framing":"medium close-up, vertical 9:16, microphone visible","voice":"warm-baritone|bright-tenor|soft-alto|crisp-soprano"},{"id":"guest","persona":"...","appearance":"clearly different from host","wardrobe":"...","setting":"set B — different props from host's set","framing":"...","voice":"..."}],"hook":"first-line cold open","scenes":[{"id":1,"speaker":"host","text":"line ≤10 words","duration":${safeClip},"imagePrompt":"detailed visual prompt with full speaker identity lock for THIS shot","videoPrompt":"motion-only ≤14 words","emphasis":"key word to italicize|null","productPop":false}]}`;
