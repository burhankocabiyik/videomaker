...
export async function generateVideo(params = {}) {
    ...
    // Seedance 2.0 expects duration as a number and accepts arbitrary seconds;
    // Kling v2.x wants '5' or '10' as a string. Dispatch on the slug so we
    // don't fight validators on either side.
    const rawDuration = params.duration != null ? String(params.duration) : null;
    if (rawDuration) {
        const clamped = model.durations.includes(rawDuration) ? rawDuration : model.durations[0];
        if (model.endpoint.startsWith('bytedance/seedance-2.0')) {
            input.duration = Number(clamped);
