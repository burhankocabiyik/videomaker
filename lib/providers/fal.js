...
export async function generateVideo(params = {}) {
    ...
    // Pick the model-supported duration that's closest to (but not less than) the
    // requested seconds. Previously we silently fell back to durations[0] when an
    // exact match wasn't in the enum — that truncated 9s requests to 5s and
    // chopped half the podcast off in the renderer.
    ...
    if (Number.isFinite(rawDuration) && model.durations.length > 0) {
        const numeric = model.durations
            .map((d) => ({ raw: d, n: Number(d) }))
            .filter((d) => Number.isFinite(d.n));
        // Prefer the smallest duration that is ≥ the request; if every option
        // is shorter, fall back to the largest available.
